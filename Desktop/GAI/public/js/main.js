;(function(){
  ...
  function card(a){
    ...
    return '<article class="main-article-item glass-panel">'
      ...
      + (pickTopic(a)?('<span class="category-badge topic-badge">'+esc(pickTopic(a)).toUpperCase()+'</span>'):'')
      + '</div>'
      + '</div>'
  ...
  function featuredHeroCard(a){
    if(!a) return '';
    var id=esc(a.id),title=esc(a.title),subtitle=esc(a.subtitle||''),cat=esc(a.category||'');
    var href = 'index.html?id='+encodeURIComponent(id);
    return '<article class="featured-card">'
      + '<div class="card-badge">'+(cat.toUpperCase()||'FEATURED')+'</div>'
      + '<h2>'+title+'</h2>'
      + (subtitle?('<p>'+subtitle+'</p>'):'')
      + '<a href="'+href+'" class="btn-primary">Read More</a>'
      + '</article>';
  }

  function featuredSideCard(a){
    if(!a) return '';
    var id=esc(a.id),title=esc(a.title),subtitle=esc(a.subtitle||'');
    var href = 'index.html?id='+encodeURIComponent(id);
    return '<article class="featured-card secondary">'
      + '<h3>'+title+'</h3>'
      + (subtitle?('<p>'+subtitle+'</p>'):'')
      + '<a href="'+href+'" class="btn-outline">Read More</a>'
      + '</article>';
  }

  function renderFeatured(list){
    var container=q('#featured-articles-container');
    if(!container) return;
    if(!list||!list.length){ container.style.display='none'; return; }
    var sorted=list.slice().sort(byDateDesc);
    var html = featuredHeroCard(sorted[0]);
    if(sorted.length > 1) html += featuredSideCard(sorted[1]);
    container.innerHTML = html;
    featuredState.ids = sorted.slice(0,2).map(function(x){ return String(x && x.id || '').trim(); }).filter(Boolean);
  ...
  function renderDetail(a){
    ...
    var html=''
      + '<article class="article-content article-detail glass-panel p-xl">'
