...
export interface SafetyRule {
  ...
  category: 'file_system' | 'system_config' | 'memory' | 'network' | 'code_execution';
  pattern: RegExp | string | ((operation: SafetyOperation) => boolean);
...
class GAISafetyService {
  ...
  private maxIncidentsBeforeLockdown = 10;
  private lockdownThreshold = 5; // critical incidents
  private isInLockdown = false;
  ...
  async checkSafety(operation: SafetyOperation): Promise<{
    safe: boolean;
    action: 'allow' | 'warn' | 'block' | 'require_approval';
    rule?: SafetyRule;
    message: string;
  ...
  private checkRule(rule: SafetyRule, operation: SafetyOperation): boolean {
    try {
  ...
  private handleRuleViolation(rule: SafetyRule, operation: SafetyOperation): {
    safe: boolean;
    action: 'allow' | 'warn' | 'block' | 'require_approval';
    rule: SafetyRule;
    message: string;
  ...
  private updateStats(incident: SafetyIncident): void {
  ...
  private checkLockdownConditions(): void {
    const recentIncidents = this.incidents.filter(incident => 
      Date.now() - incident.timestamp < 5 * 60 * 1000 // 5 minut
    );
  ...
  private enterLockdown(): void {
    if (this.isInLockdown) return;
    ...
    const lockdownIncident: SafetyIncident = {
  ...
  getRecentIncidents(limit = 10): SafetyIncident[] {
    return [...this.incidents]
      .sort((a, b) => b.timestamp - a.timestamp)
      .slice(0, limit);
  }

  getUnresolvedIncidents(): SafetyIncident[] {
...
// 🎯 FUNKCJE POMOCNICZE
export const createSafeOperation = (type: string, target: string, content?: string): SafetyOperation => ({
...
export const checkOperationSafety = async (operation: SafetyOperation): Promise<{
  safe: boolean;
  action: string;
  message: string;
  details?: string;
