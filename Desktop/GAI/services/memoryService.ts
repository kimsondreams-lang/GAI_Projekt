...
class MemoryService {
  ...
  private readonly MIN_FETCH_INTERVAL = 500;

  constructor() {
    this.db = this.getEmptyState();
    // Browser-only event listeners - skip in Node.js environment
  ...
  private getEmptyState(): DatabaseSchema {
    return {
      ...
      settings: {
        ...
        ftpConfig: { host: '', user: '', port: '21', pass: '', rootPath: '/public_html/kimsondreams' },
        telegramConfig: { botToken: '', chatId: '', enabled: false },
        heartbeat: { enabled: true, intervalSeconds: 20 },
        autonomyEnabled: true,
        autonomyScheduler: 'heartbeat',
        ...
        // --- DEFAULT AGENTIC CONFIG ---
        agenticSystem: {
          enabled: false,
          masterModel: '',
          allowedModels: [], // User must select which models are available to the brain
          modules: []
        }
      },
