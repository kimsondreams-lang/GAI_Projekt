;(function(){
  ...
  async function loadArticles() {
      try {
          ...
          try {
              ...
              if (response.ok) {
                  const filenames = await response.json(); // ["article1.json", "2025-08-18.json", ...]
                  
                  const promises = filenames.map(file => 
                      fetch(`data/articles/${file}`).then(r => r.json()).catch(e => null)
                  );
              ...
              // Fallback to legacy global var if defined
  ...
  function card(a){
    ...
    if (img && !img.startsWith('http') && !img.startsWith('/') && !img.startsWith('images/')) img = 'images/articles/' + img;
    
    return '<article class="main-article-item">'
      + '<div class="article-image">'
      + (img?('<img src="'+img+'" alt="'+title+'" loading="lazy">'):'')
      + (cat?('<span class="category-tag glass-badge glass-badge--primary corner-badge corner-badge--tl">'+cat+'</span>'):'')
      + '</div>'
      + '<div class="article-content">'
      + '<h3 class="article-title"><a href="index.html?id='+encodeURIComponent(id)+'">'+title+'</a></h3>'
      ...
      + '<a class="read-more-btn" href="index.html?id='+encodeURIComponent(id)+'">Read more</a>'
      + '</div>'
      + '</div>'
      + '</article>'
  }
  ...
  function normalizeImageSrc(img){
    ...
    if(!s) return '';
    if (s.startsWith('http') || s.startsWith('/') || s.startsWith('images/')) return s;
    return 'images/articles/' + s;
  }
  ...
  function secondary(a){
    ...
    return '<div class="secondary-article">'
      + '<div class="article-image">'+(img?('<img src="'+img+'" alt="'+title+'" loading="lazy">'):'')+'</div>'
      + '<div class="article-content">'
  ...
  var mainState = { sorted: [], shown: 0, batch: 10, observer: null };
  ...
  function renderDetail(a){
    ...
    var galleryHtml=gal.length?('<div class="article-gallery">'+gal.map(function(g){ 
        var s=esc(g.src); 
        if (s && !s.startsWith('http') && !s.startsWith('/') && !s.startsWith('images/')) s = 'images/articles/' + s;
