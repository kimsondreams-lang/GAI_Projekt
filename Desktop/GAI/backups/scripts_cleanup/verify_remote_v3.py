...
def verify():
    try:
        ...
        # Check main index (relative path)
        bio_idx = io.BytesIO()
        ftps.retrbinary('RETR articles.json', bio_idx.write)
        idx_content = bio_idx.getvalue().decode('utf-8')
        
        # Check specific article (relative path)
        bio_art = io.BytesIO()
        ftps.retrbinary('RETR articles/best-tech-gadgets-amazon-2025.json', bio_art.write)
        ...
        jpg_in_art = '.jpg' in art_content or '.png' in art_content
        
        print('--- Remote Verification Results ---')
        print(f'S25 Ultra in index: {s25_in_idx}')
        print(f'S24 Ultra in article: {s24_in_art}')
        print(f'WebP references in article: {webp_in_art}')
        print(f'Legacy extensions (jpg/png) in article: {jpg_in_art}')
