...
4. JSON podawaj tylko tam, gdzie narzędzie tego wymaga. FILE_READ przyjmuje string, WEB_SEARCH/WEB_READ akceptują string lub obiekt.
5. Dla WEB_READ używaj pełnych URL z https://.
6. Unikaj używania SHELL do zapisu JSON przez python heredoc; preferuj FILE_WRITE/BLOG_PUBLISH lub PYTHON_EXEC.
...
- \`[[FILE_WRITE: {"path": "...", "content": "..."}]]\` - Write or overwrite a file. Dla JSON możesz podać content jako obiekt.
- \`[[FILE_REPLACE: {"path": "...", "oldStr": "...", "newStr": "..."}]]\` - Replace specific text in a file.
- \`[[FILE_READ: <path>]]\` - Read file content.
- \`[[WEB_SEARCH: "query"]]\` - Search the web (string or {"query": "..."}).
- \`[[WEB_READ: "url"]]\` - Read web page (string or {"url": "https://..."}).
- \`[[PYTHON_EXEC: {"code": "..."}]]\` - Execute Python code.
- \`[[VISION_ANALYZE: {"path": "...", "prompt": "..."}]]\` - Analyze image.
- \`[[DEV_BUILD: {}]]\` - Run npm build.
- \`[[BLOG_PUBLISH: {"fileName":"...", "article":{...}}]]\` - Publish article.
...
8. JSON tylko tam, gdzie narzędzie tego wymaga. FILE_READ przyjmuje ścieżkę jako string. WEB_SEARCH/WEB_READ akceptują string lub obiekt, WEB_READ wymaga pełnego https:// URL. Unikaj używania SHELL do zapisu JSON przez python heredoc; preferuj FILE_WRITE/BLOG_PUBLISH lub PYTHON_EXEC. FILE_WRITE może przyjąć content jako obiekt JSON.
