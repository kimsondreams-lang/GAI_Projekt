...
class OpenAIProvider:
    def __init__(self, api_key: str, organization: Optional[str] = None):
        """
        Inicjalizacja OpenAI Provider z sync client (dla kompatybilności)
    ...
    async def generate_text(
        ...
        **kwargs
        ...
        """
        Generowanie tekstu z OpenAI API (async wrapper dla sync client)
        ...
        try:
            ...
            # Wywołanie API w wątku (aby nie blokować)
    ...
    async def generate_embedding(
        ...
        """
        Generowanie embeddingu z OpenAI (async wrapper dla sync client)
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        """
        try:
            # Proste testowe wywołanie w wątku
            loop = asyncio.get_event_loop()
            response = await loop.run_in_executor(
                None,
                lambda: self.client.chat.completions.create(
