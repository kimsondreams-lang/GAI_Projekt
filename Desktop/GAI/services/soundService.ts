...
const themeProfiles: Record<string, Record<SoundType, SoundProfile>> = {
    real: {
        click: { wave: 'file', fileUrl: '/sounds/click.wav' },
        hover: { wave: 'file', fileUrl: '/sounds/hover.wav' },
        success: { wave: 'file', fileUrl: '/sounds/success.wav' },
        error: { wave: 'file', fileUrl: '/sounds/error.wav' },
        startup: { wave: 'file', fileUrl: '/sounds/startup.wav' }
    },
    neu: {
...
class SoundService {
    ...
    play(type: SoundType) {
        ...
        // Synthesis
        if (!p.f0 || !p.duration || !p.g0) return; // Basic validation for synth
